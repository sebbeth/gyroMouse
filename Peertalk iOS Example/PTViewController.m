...
- (void)viewDidLoad {
  [super viewDidLoad];
  //gyro stuff
    gyroscopeManager = [[CMMotionManager alloc] init];
    gyroscopeManager.deviceMotionUpdateInterval = 0.0001;
    [gyroscopeManager startDeviceMotionUpdates];
  ...
  [channel listenOnPort:PTExampleProtocolIPv4PortNumber IPv4Address:INADDR_LOOPBACK callback:^(NSError *error) {
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.001 target:self selector:@selector(gyroLoop) userInfo:nil repeats:YES];
...
- (void)sendMessage {
  if (peerChannel_) {
    dispatch_data_t payload = PTExampleTextDispatchDataWithString(messageToSend);
    ...
    [self appendOutputMessage:[NSString stringWithFormat:@"%@", messageToSend]];
  } else {
...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData*)payload {
  ...
  if (type == PTExampleFrameTypeTextMessage) {
    ...
    [self appendOutputMessage:[NSString stringWithFormat:@"[%@]: %@", channel.userInfo, messageToSend]];
...
-(void)gyroLoop {
    [gyroscopeManager startDeviceMotionUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMDeviceMotion *deviceMotion, NSError *error) {
        /*z_Rotation_True = deviceMotion.gravity.z * (180/M_PI);
        z_Rotation = z_Rotation_True + [userDefaults floatForKey:@"PMcalibrationZ_Rotation"];
        x_Rotation_True = -(deviceMotion.gravity.y * (180/M_PI));
        x_Rotation = x_Rotation_True + [userDefaults floatForKey:@"PMcalibrationX_Rotation"];
        y_Rotation_True = deviceMotion.gravity.x * (180/M_PI);
        y_Rotation = y_Rotation_True + [userDefaults floatForKey:@"PMcalibrationY_Rotation"];*/
        cursorXPosition = 2 * (deviceMotion.gravity.x * (180/M_PI));
        messageToSend = [NSString stringWithFormat:@"X%f", cursorXPosition];
        [self sendMessage];
        cursorYPosition = 2 * (deviceMotion.gravity.y * (180/M_PI));
        messageToSend = [NSString stringWithFormat:@"Y%f", cursorYPosition];
        [self sendMessage];
